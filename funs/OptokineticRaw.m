...
function result = OptokineticRaw(fileName)
...
for(i = 3 : length(data.gazeRaw(:,1)))
  gazeVelRaw(i-1,1) = 1000*(data.gazeRaw(i,1)-data.gazeRaw(i-1,1))/(data.time(i)-data.time(i-1));
  gazeVelRaw(i-1,2) = 1000*(data.gazeRaw(i,2)-data.gazeRaw(i-1,2))/(data.time(i)-data.time(i-1));
end
...
[velPks,velPksMag] = peakIdentification(gazeVelRaw.',60,30);

%figure;
%hold on;
%plot(data.time,gazeRaw);
%scatter(data.time(velPks),gazeRaw(velPks));
%Temporal Plots
%figure;
%hold on;
%plot(data.time/(max(data.time)-min(data.time)),gazeRaw/(max(gazeRaw)-min(gazeRaw)),'b','LineWidth',1.5);
%plot(data.time/(max(data.time)-min(data.time)),gazeVelRaw/(max(gazeVelRaw)-min(gazeVelRaw)),'r','LineWidth',1.5);
%scatter(data.time(velPks)/(max(data.time)-min(data.time)),gazeVelRaw(velPks)/(max(gazeVelRaw)-min(gazeVelRaw)),'b','filled');
%figure;
%hold on;
%plot(data.time,gazeVelRaw,'b','LineWidth',1.5);
%scatter(data.time(velPks),gazeVelRaw(velPks),'r','filled');
...
%Funcion para unir los pares de movimiento de baja velocidad (Pursuit)
ind = 1;
numPursuits = 0;
...
%showPSAngle=gazePursuitAngle
%showNYAngle=gazeNystagmusAngle
...
%showMin=gazeRaw(gazeRawMin)
%showMax=gazeRaw(gazeRawMax)
...
for(i = 1 : length(pursuit(:,1))) 
  if(rCoef(i) >= 0.90)     
    %amplitud del seguimiento lento
...
for i=1:length(nystagmus)
  %Amplitud del nistagmo optocinetico
  if sign(gazeRaw(nystagmus(i,2))*gazeRaw(nystagmus(i,1)))<0
    amplitudeNY(i) = abs(gazeRaw(nystagmus(i,2))-gazeRaw(nystagmus(i,1)));
  else
    amplitudeNY(i) = abs(gazeRaw(nystagmus(i,2)))+abs(gazeRaw(nystagmus(i,1)));
  end
  %Velocidad media de los nistagmos optocineticos
  meanNYVel(i) = mean(gazeVel(nystagmus(i,1):nystagmus(i,2))); 
end
...
%for(i = 1 : length(pursuit(:,1)))
%  %Amplitud del nistagmo optocinetico
%  NysAmplitude(ind) = gazeRaw(gazeRawMax(i)) - gazeRaw(gazeRawMin(i));
%  if(rCoef(i) >= 0.90)
%    %amplitud del seguimiento lento
%    SPAmplitude(ind)= abs(gazeRaw(pursuit(i,2)-pursuit(i,1)));
%    %Velocidad media de los seguimientos de las barras
%    meanSlowVel(ind) = mean(gazeVel(pursuit(i,1):pursuit(i,2)));
%   
%    ind += 1;
...
%showmeanVelPS=meanSlowVel
%showmeanVelNY=meanFastVel
%figure;
%hold on;
%title("XY Gaze");
%xlabel("X Gaze /deg");
...
%for(i = 1 : length(meanVelWindow(:,1)))
%  scatter(data.time(meanVelWindow(i,:)),gazeVelRaw(meanVelWindow(i,:)),[],cmap(i,:),'filled');
%  scatter(data.time(maxVelRawInd),gazeVelRaw(maxVelRawInd),'r','filled');
%end
...
meanFastVelMean=mean(meanNYVel);
if length(meanNYVel)!=1
  meanFastVelSD=std(meanNYVel);
else
  meanFastVelSD=NaN;
endif
