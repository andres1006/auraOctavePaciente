...
function [finalFixations] = memory_finalFixation (indexInitialPoints, indexFixations, time, gaze, indexStimChange, stimulus)
  %defines
  minimaDuracion=200;
  maxDuration=0;
  fixationEnd=1;
...
%      maxDuration=time(finalFixationsEnd(i))-time(finalFixationsStart(i));
      ...
      while(indexFixations(fixationEnd,2)<=compareValue || i==length(indexInitialPoints) && fixationEnd<=length(indexFixations(:,1)))
        if indexFixations(fixationEnd,1)>=indexInitialPoints(i) %El comienzo de la fijacion debe estar despues del punto inicial
          for ii=indexFixations(fixationEnd,1):indexFixations(fixationEnd,2)
            if stimulus(ii)!=0
              stim=stimulus(ii);
              break;
            elseif ii==indexFixations(fixationEnd,2)
              stim=0;
            endif
          endfor
          if(time(indexFixations(fixationEnd,2))-time(indexFixations(fixationEnd,1))) > maxDuration && (time(indexFixations(fixationEnd,2))-time(indexFixations(fixationEnd,1))) > minimaDuracion && (stim*mean(gaze(indexFixations(fixationEnd,1):indexFixations(fixationEnd,2)))>0)%Prevalece la que tiene mas duracion
